...
if($i && is_numeric($i)){
	...
	$query = mysql_query("SELECT * FROM `alerts` WHERE `id`= '$i'");
		...
		<?
			if($subject != 'placeholder' || $subject == ""){
				if($method == 'txt'){
					...
					echo "Text style will go here, eventually";
				} elseif($errormsg) {
					echo $errormsg;
				}else{
		?>
			...
			<div style='text-align:right; float:right; '><font style='font-size:.65em'>Posted on <? echo $date ?></font></div>
